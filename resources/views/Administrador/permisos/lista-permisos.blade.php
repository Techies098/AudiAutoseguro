        ...
        <form action="{{ route('administrador/permisos.guardar', ['user' => $user->id]) }}" method="POST">
            ...
            @foreach ($permissions as $permission)
                <li class="list-group-item">
                    <input class="form-check-input mr-2" type="checkbox" value="{{ $permission->id }}" id="permission_{{ $permission->id }}" name="permissions[]" @if(in_array($permission->id, $allUserPermissions)) checked disabled @endif>
